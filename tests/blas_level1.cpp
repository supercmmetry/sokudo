...
TEST(BlasLevel1Test, CUDASasum1) {
    ...
    auto task = sokudo::kernels::blas::Sasum<sokudo::CUDA>()(buf_a, incx, res);
    task->sync();
...
TEST(BlasLevel1Test, CUDADasum1) {
    ...
    auto task = sokudo::kernels::blas::Dasum<sokudo::CUDA>()(buf_a, incx, res);
    task->sync();
...
TEST(BlasLevel1Test, OpenCLSasum1) {
    ...
    auto task = sokudo::kernels::blas::Sasum<sokudo::OPENCL>()(buf_a, incx, res);
    task->sync();
    ASSERT_TRUE(res == (float) 1048576.0);
...
TEST(BlasLevel1Test, OpenCLDasum1) {
    ...
    auto task = sokudo::kernels::blas::Dasum<sokudo::OPENCL>()(buf_a, incx, res);
    task->sync();
    ASSERT_TRUE(res == 1048576.0);
