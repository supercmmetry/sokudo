...
CudaAbstractTask cu_samax(float *a, uint64_t *res, uint64_t n, uint64_t incx) {
    ...
    int r = 0;
    berr << cublasIsamax_v2(handle, p, dx, incx, &r);
    *res = r;
...
CudaAbstractTask cu_damax(double *a, uint64_t *res, uint64_t n, uint64_t incx) {
    ...
    auto p = n / incx + (n % incx != 0);
    int r = 0;
    berr << cublasIdamax_v2(handle, p, dx, incx, &r);
    *res = r;
    return CudaAbstractTask(stream, handle) << dx;
}
...
CudaAbstractTask cu_scamax(void *a, uint64_t *res, uint64_t n, uint64_t incx) {
    ...
    auto p = n / incx + (n % incx != 0);
    int r = 0;
    berr << cublasIcamax_v2(handle, p, reinterpret_cast<const cuComplex *>(dx), incx, &r);
    *res = r;
    return CudaAbstractTask(stream, handle) << dx;
}
...
CudaAbstractTask cu_dcamax(void *a, uint64_t *res, uint64_t n, uint64_t incx) {
    ...
    auto p = n / incx + (n % incx != 0);
    int r = 0;
    berr << cublasIzamax_v2(handle, p, reinterpret_cast<const cuDoubleComplex *>(dx), incx, &r);
    *res = r;
    return CudaAbstractTask(stream, handle) << dx;
}
