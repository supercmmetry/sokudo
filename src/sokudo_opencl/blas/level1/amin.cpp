...
sokudo::CLTask *
sokudo::opencl::kernels::blas::cl_samin(const sokudo::Buffer<float> &x, const sokudo::Value<uint64_t> &incx,
    ...
    detail_amin_cl(task, "BLAS_SAMIN", x.name(), incx.name(), res.name(), local_size, s);
    return task;
}

sokudo::CLTask *
sokudo::opencl::kernels::blas::cl_damin(const sokudo::Buffer<double> &x, const sokudo::Value<uint64_t> &incx,
                                        const sokudo::Value<uint64_t> &res, uint64_t wgs, uint64_t stride) {
    // register kernel
    register_amin();
    ...
    auto task = new CLTask(queue);
    detail_amin_cl(task, "BLAS_DAMIN", x.name(), incx.name(), res.name(), local_size, s);
    return task;
}

sokudo::CLTask *
sokudo::opencl::kernels::blas::cl_scamin(const sokudo::Buffer<float2> &x, const sokudo::Value<uint64_t> &incx,
                                         const sokudo::Value<uint64_t> &res, uint64_t wgs, uint64_t stride) {
    // register kernel
    register_amin();
    ...
    queue.enqueueReadBuffer(buf_b, CL_FALSE, 0, res.bsize(), res.inner());
    auto task = new CLTask(queue);
    detail_amin_cl(task, "BLAS_SCAMIN", x.name(), incx.name(), res.name(), local_size, s);
    return task;
}

sokudo::CLTask *
sokudo::opencl::kernels::blas::cl_dcamin(const sokudo::Buffer<double2> &x, const sokudo::Value<uint64_t> &incx,
                                         const sokudo::Value<uint64_t> &res, uint64_t wgs, uint64_t stride) {
    // register kernel
    register_amin();
    ...
    auto task = new CLTask(queue);
    detail_amin_cl(task, "BLAS_DCAMIN", x.name(), incx.name(), res.name(), local_size, s);
    return task;
}
