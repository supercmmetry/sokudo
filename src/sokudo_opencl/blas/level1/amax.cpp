...
sokudo::opencl::kernels::blas::cl_samax(
    ...
    detail_amax_cl(task, "BLAS_SAMAX", _n.name(), x.name(), incx.name(), res.name(), local_size, s);
    return task;
}

sokudo::CLTask *
sokudo::opencl::kernels::blas::cl_damax(
        const sokudo::Value<uint64_t> &_n,
    ...
    auto task = new CLTask(queue);
    detail_amax_cl(task, "BLAS_DAMAX", _n.name(), x.name(), incx.name(), res.name(), local_size, s);
    return task;
}

sokudo::CLTask *
sokudo::opencl::kernels::blas::cl_scamax(
        const sokudo::Value<uint64_t> &_n,
    ...
    queue.enqueueReadBuffer(buf_b, CL_FALSE, 0, res.bsize(), res.inner());
    auto task = new CLTask(queue);
    detail_amax_cl(task, "BLAS_SCAMAX", _n.name(), x.name(), incx.name(), res.name(), local_size, s);
    return task;
}

sokudo::CLTask *
sokudo::opencl::kernels::blas::cl_dcamax(
        const sokudo::Value<uint64_t> &_n,
    ...
    auto task = new CLTask(queue);
    detail_amax_cl(task, "BLAS_DCAMAX", _n.name(), x.name(), incx.name(), res.name(), local_size, s);
    return task;
}
